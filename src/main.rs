...
use classify::api::{AppState, start_server};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let content_storage = match create_content_storage(&config.storage.storage_type, &config.storage).await {
        Ok(storage) => storage,
        Err(e) => {
    ...
    info!("Content storage initialized: {:?}", config.storage.storage_type);

    // Create tag storage
    let tag_storage = match create_tag_storage(&config.tag_storage.tag_storage_type, &config.tag_storage).await {
        Ok(storage) => storage,
        Err(e) => {
    ...
    info!("Tag storage initialized: {:?}", config.tag_storage.tag_storage_type);

    // Create classifier
    let classifier = match create_classifier(&config.classifier.classifier_type, &config.classifier).await {
    ...
    info!("Classifier initialized: {:?}", config.classifier.classifier_type);
